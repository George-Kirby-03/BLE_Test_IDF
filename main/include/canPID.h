...
#define CAN_PID_LIST_INT(pid, function, id_name) { .name = id_name, .i_data = 0, .PID_index = pid, .i_gen_func = function, .is_available = 1, .is_float = 0,  }
#define CAN_PID_LIST_FLOAT(pid, function, id_name) { .name = id_name, .f_data = 0.0f, .PID_index = pid, .f_gen_func = function, .is_available = 1, .is_float = 1,  }
...
/**
 * @brief Initializes the CAN bus with the specified configurations.
 * 
 * @param car_settings Pointer to the CAN_Data_handler structure.
 * @param t_config Pointer to the timing configuration.
 * @param filter_config Pointer to the filter configuration.
 * @param general_config Pointer to the general configuration.
